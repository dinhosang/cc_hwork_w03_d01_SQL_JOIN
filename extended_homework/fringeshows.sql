...
-- select fields to display join imprtant features and can p
...
SELECT name, COUNT(*) FROM shows_users INNER JOIN users on users.id = shows_users.user_id GROUP BY users.name;
--   21. SELECT all users who are going to a show at 17:15.
SELECT users.name FROM users INNER JOIN shows_users ON users.id = shows_users.user_id INNER JOIN times ON shows_users.show_id = times.show_id WHERE time = '17:15' GROUP BY users.name; -- the group by removes duplicates
